...
export default function JobIndustryTree() {
    ...
    if (jobrole) {
      ...
      try {
        const response = await fetch(
          `http://localhost:5000/tasks?jobrole=${jobrole}`
        );
        if (!response.ok) {
  ...
  const handleSkillClick = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/skills?jobrole=${selectedJobRole}`
      );
      if (!response.ok) {
        throw new Error("Failed to fetch skills");
      }
      const data = await response.json();
      const skillList = data.map((skill: { skill: string }) => skill.skill); // Extract skill names
      setSkills(skillList); // Store skills in state
      setView("skill"); // Set view to Skill
    } catch (err) {
      if (err instanceof Error) {
        setError(err.message);
      } else {
